import {ControllerMiddleware, datastore, Model, ObjectId, PolicyController} from '@bim/deco-api';
...
export class ThreeModuleControllerMiddleware extends ControllerMiddleware {
    ...
    public async setModelFromModuleType(elementId: string | ObjectId, req: Request, res: Response): Promise<string | ObjectId> {
        ...
        const data = await datastore.db.collection(this.model.deco.collectionName).findOne({_id: elementId});
