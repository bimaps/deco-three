...
export class ThreeCoreControllerMiddleware extends PolicyController {
  ...
  public importJSON(req: Request, res: Response, next: NextFunction) {
    if (!res.locals.element)
      return next(
        new Error(
          "Import JSON requires to first fetch a site in res.locals.element"
        )
      );
    let rightInstance = res.locals.element instanceof ThreeSiteModel;
    if (!rightInstance)
      return next(
        new Error("res.locals.element is not a ThreeSiteModel instance")
      );
    ...
    if (!json.metadata)
      return next(
        new Error(
          "Missing metadata in json, are you sure you upload the right JSON format?"
        )
      );
    if (!json.metadata.version || parseFloat(json.metadata.version) !== 4.5)
      return next(
        new Error(
          "Invalid or not supported version. We currently support version 4.5"
        )
      );

    let importId: string | undefined =
      typeof req.query.importId === "string" ? req.query.importId : undefined;
  ...
  public importIFC(req: Request, res: Response, next: NextFunction) {
    if (!res.locals.element)
      return next(
        new Error(
          "Import IFC requires to first fetch a site in res.locals.element"
        )
      );
    let rightInstance = res.locals.element instanceof ThreeSiteModel;
    if (!rightInstance)
    ...
    if (req.file && req.file.path) {
      ...
      IfcHelper.convert2obj(req.file.path)
        ...
        .then((result) => {
          if (!result)
            throw new Error(
              "No result when converting and loading IFC in obj/mtl"
            );
          if (typeof result !== "boolean" && result.type) {
            let json: ThreeJsonData = result.toJSON();
            let importId: string | undefined =
              typeof req.query.importId === "string"
                ? req.query.importId
                : undefined;
            let saveLights = req.query.saveLights ? true : false;
        ...
        .then(() => {
          Operation.completeCurrentOperation(
            res,
            "completed",
          ...
          if (req.query.reportId && req.query.email) {
            ActionsService.runActions(
              res,
              [
                [
                  ThreeReportAction,
                  ThreeSendReportAction,
                  ThreeDeleteSiteAction,
                ],
              ],
              {
                siteId: site._id,
  ...
  public fetchBuildingsInfos() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      new Promise(async (resolve, reject) => {
        try {
          const buildings = await ThreeBuildingModel.getAll(
          ...
          const buildingsOutput = await ThreeBuildingModel.outputList(
            buildings
          );
  ...
  public fetchKeyValues() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      new Promise(async (resolve, reject) => {
        try {
          const objects = await ThreeObjectModel.getAll(
            new Query({ siteId: site._id })
          );
          const buildings = await ThreeBuildingModel.getAll(
            new Query({ siteId: site._id })
            ...
            for (let objKey of objKeys) {
              ...
              if (
                ...
                for (let level2Path of level2Paths) {
                  if (
                    !extractPaths.includes(level2Path) &&
                    !extractPaths.includes(`${objKey}.*`)
                  ) {
                    continue;
                  }
                  const valueLevel2 = resolvePath(
                    obj,
                    preparePathKey(level2Path)
                  );
                  if (
                    typeof valueLevel2 === "string" ||
                    typeof valueLevel2 === "number" ||
                    typeof valueLevel2 === "boolean"
                  ) {
                    ...
                    const level3Paths = level3Keys.map(
                      (k) => `${level2Path}.${k}`
                    );

                    for (let level3Path of level3Paths) {
                      const valueLevel3 = resolvePath(
                        obj,
                        preparePathKey(level3Path)
                      );
                      if (
                        typeof valueLevel3 === "string" ||
                        typeof valueLevel3 === "number" ||
                        typeof valueLevel3 === "boolean"
                      ) {
