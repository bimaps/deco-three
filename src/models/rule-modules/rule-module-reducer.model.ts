...
export class RuleModuleReducerModel
  extends RuleModuleBaseModel
  implements RuleModuleReducer
{
  @type.id
  public _id: ObjectId;
  ...
  public async process(flow: RuleModel): Promise<void> {
    ...
    if (
      ...
      if (this.currentInputType === "numbers") {
        ...
        throw new Error(
          "Min, max, average and sum reducers modules only accepts number, numbers, string and strings as input"
        );
      }
      ...
      if (this.operation === "min") {
        ...
        const refKeys = [...Object.keys(mathInput)].filter(
          (i) => mathInput[parseInt(i, 10)] === min
        );
        this.outputValue = min;
        const inputRefs = (this.currentInputRef as RuleModuleIORef[]) || [];
        this.outputReference = inputRefs.filter((v, i) =>
          refKeys.includes(i.toString())
        );
        ...
        this.outputValue =
          mathInput.reduce((a, b) => a + b, 0) / mathInput.length;
        this.outputReference = this.currentInputRef;
      }
