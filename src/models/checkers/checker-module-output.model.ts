...
export class CheckerModuleOutputModel
  extends RuleModuleBaseModel
  implements RuleModuleOutput
{
  @type.id
  public _id: ObjectId;
  ...
  public async process(flow: RuleModel): Promise<void> {
    ...
    for (const outputConfig of this.outputs) {
      ...
      if (Array.isArray(inputValueType.value)) {
        if (
          Array.isArray(inputValueType.ref) &&
          inputValueType.ref.length !== inputValueType.value.length
        ) {
          throw new Error(
            "Ref array length must be the same than value array length"
          );
        }
        if (
          Array.isArray(inputValueType.style) &&
          inputValueType.style.length !== inputValueType.value.length
        ) {
          throw new Error(
            "Style array length must be the same than value array length"
          );
        }
        for (let index = 0; index < inputValueType.value.length; index++) {
          const value = inputValueType.value[index];
          const ref = Array.isArray(inputValueType.ref)
            ? inputValueType.ref[index]
            : inputValueType.ref;
          const style = Array.isArray(inputValueType.style)
            ? inputValueType.style[index]
            : inputValueType.style;
          const type: any =
  ...
  public async summary(): Promise<void> {
    ...
    if (Array.isArray(this.outputValue)) {
      out = JSON.stringify(
        (this.outputValue as any[]).map((ov) =>
          Object.assign({}, ov, { ref: undefined })
        )
      );
    } else if (typeof this.outputValue === "object") {
      out = JSON.stringify(
        Object.assign({}, this.outputValue, { ref: undefined })
      );
    }
