...
export class ThreeImporterHelper {
  ...
  public start(
    ...
    const reducingMaterialStats = ThreeMaterialHelper.reduceMaterials(
      this.jsonData
    );
  ...
  public rotate90X(object: THREE.Object3D): THREE.Object3D {
    ...
    for (const child of object.children || []) {
      const translation = child.position
        .clone()
        .sub(point)
        .projectOnPlane(axis);
  ...
  public removeImport(
    ...
    deletePromises.push(
      ThreeObjectModel.deco.db
        .collection(ThreeObjectModel.deco.collectionName)
        .deleteMany(query)
    );
    deletePromises.push(
      ThreeMaterialModel.deco.db
        .collection(ThreeMaterialModel.deco.collectionName)
        .deleteMany(query)
    );
    deletePromises.push(
      ThreeGeometryModel.deco.db
        .collection(ThreeGeometryModel.deco.collectionName)
        .deleteMany(query)
    );
    deletePromises.push(
      ThreeBuildingModel.deco.db
        .collection(ThreeBuildingModel.deco.collectionName)
        .deleteMany(query)
    );
    deletePromises.push(
      ThreeStoreyModel.deco.db
        .collection(ThreeStoreyModel.deco.collectionName)
        .deleteMany(query)
    );
    deletePromises.push(
      ThreeSpaceModel.deco.db
        .collection(ThreeSpaceModel.deco.collectionName)
        .deleteMany(query)
    );
    ...
    for (let geometry of this.jsonData.geometries) {
      ...
      if (
        ...
        for (let verticeIndex in geometry.data.vertices) {
          geometry.data.vertices[verticeIndex] =
            geometry.data.vertices[verticeIndex] * this.scaleFactor;
        }
      }
  ...
  private parseObject(
    object: ThreeJsonObject,
    userData?: { [key: string]: any },
    parent: ThreeJsonObject | null = null
  ) {
  ...
  private roundGeometriesValues(): void {
    for (let geometry of this.geometries) {
      if (
        ...
        for (let verticeIndex in geometry.data.vertices) {
          geometry.data.vertices[verticeIndex] =
            ThreeImporterHelper.roundGeometryValue(
              geometry.data.vertices[verticeIndex]
            );
        }
        ...
        if (geometry.data?.attributes?.position) {
          for (
            let index = 0;
            index < geometry.data.attributes.position.array.length;
            index++
          ) {
            geometry.data.attributes.position.array[index] =
              ThreeImporterHelper.roundGeometryValue(
                geometry.data.attributes.position.array[index]
              );
          }
        ...
        if (geometry.data?.attributes?.normal) {
          for (
            let index = 0;
            index < geometry.data.attributes.normal.array.length;
            index++
          ) {
            geometry.data.attributes.normal.array[index] =
              ThreeImporterHelper.roundGeometryValue(
                geometry.data.attributes.normal.array[index]
              );
          }
  ...
  private saveMaterials(): Promise<any> {
        ...
        ThreeMaterialModel.instanceFromDocument(material)
          .then((materialInstance) => {
            return materialInstance
              .toDocument("insert")
              .then((doc) => doc.getInsertDocument());
          })
          .then((document) => {
    ...
    return Promise.all(materialPromises).then((documents) => {
      return ThreeMaterialModel.deco.db
        .collection(ThreeMaterialModel.deco.collectionName)
  ...
  private saveGeometries(): Promise<any> {
    ...
    return Promise.all(geometryPromises).then((documents) => {
      return ThreeGeometryModel.deco.db
        .collection(ThreeGeometryModel.deco.collectionName)
        .insertMany(documents);
    });
  }
  ...
  private saveObjects(): Promise<any> {
    ...
    return Promise.all(objectPromises).then((documents) => {
      return ThreeObjectModel.deco.db
        .collection(ThreeObjectModel.deco.collectionName)
        .insertMany(documents);
    });
  }
